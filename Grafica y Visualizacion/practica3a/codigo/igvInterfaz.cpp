...
void igvInterfaz::set_glutKeyboardFunc(unsigned char key, int x, int y) {
	switch (key) {
	case '1':
		interfaz.escena.malla->set_aparA(interfaz.escena.malla->get_aparA() ? false : true);
		break;
	case '2':
		interfaz.escena.malla->set_aparB(interfaz.escena.malla->get_aparB() ? false : true);
		break;
	case '3':
		interfaz.escena.malla->set_aparD(interfaz.escena.malla->get_aparD() ? false : true);
		break;
    case 'x': // Apartado C: rotar X positivo 
		interfaz.escena.malla->rotarX(10);
		break;
    case 'X': // Apartado C: rotar X negativo 
		interfaz.escena.malla->rotarX(-10);
		break;
    case 'y': // Apartado C: rotar Y positivo 
		interfaz.escena.malla->rotarY(10);
		break;
    case 'Y': // Apartado C: rotar y negativo 
		interfaz.escena.malla->rotarY(-10);
		break;
    case 'z': // Apartado C: rotar z positivo 
		interfaz.escena.malla->rotarZ(+10);
		break;
